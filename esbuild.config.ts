...
import { wasmPlugin } from "./esbuild-plugin/wasm/index.ts";
  ...
  setup(build) {
    ...
    build.onEnd((result) => {
      ...
      result.outputFiles.forEach((outputFile) => {
        ...
        const slicedContent = JSON.stringify(content, null, 2).slice(1, -1);
 ...
 * 
 */
export const code = "(async () => {${slicedContent}})()";
`;

        // save wrappedContent to file
        fs.writeFileSync("code.js", wrappedContent);
...
const promises = configs.map((config) => {
  return esbuild.build({
    ...
    external: ["ethers"],
    format: "esm",
    plugins: [wasmPlugin, wrapIIFEInStringPlugin],
    inject: config?.injectedValues?.filter((value) => value !== null), // TODO: inject wasm shim
  });
});

...
export async function generateGlueCode(
  ...
  const initCode = `const __vite__wasmModule = await ${
    names.initWasm
  }(${codegenSimpleObject(importObject)}, ${names.wasmUrl});`;

  const exportsStatements = exports.map((name) => {
    return `export ${
      name === "default" ? "default" : `const ${name} =`
    } __vite__wasmModule.${name};`;
  });
...
function codegenSimpleObjectKeyValue(obj: SimpleObject): string {
  return obj
    .map(({ key, value }) => {
      return `${key}: ${
        typeof value === "string" ? value : codegenSimpleObject(value)
      }`;
    })
    .join(",\n");
}
